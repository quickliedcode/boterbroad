        ...
        if(answer["result"].size())
            if(!gbot.sendKnownPhrase(answer)){      
                ...
                if(answer["result"][0]["message"]["forward_sender_name"].is_null() && answer["result"][0]["message"]["forward_from_chat"].is_null()){
                    ...
                    gbot.sendMessage("@growbucket", "🏅 Отправил: <a href='tg://user?id=" + to_string(answer["result"][0]["message"]["from"]["id"])+ "'>" + string(answer["result"][0]["message"]["from"]["first_name"]) + " " + string(answer["result"][0]["message"]["from"]["last_name"]) + "</a>");
                }
                else{
