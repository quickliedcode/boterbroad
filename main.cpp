        ...
        if(answer["result"].size())
            if(!gbot.sendKnownPhrase(answer)){      
                ...
                if(answer["result"][0]["message"]["forward_sender_name"].is_null() && answer["result"][0]["message"]["forward_from_chat"].is_null()){
                    ...
                    gbot.sendMessage("@growbucket", "🏅 Отправил: " + "[" + string(answer["result"][0]["message"]["from"]["first_name"]) + " " + string(answer["result"][0]["message"]["from"]["last_name"]) + "](@" + string(answer["result"][0]["message"]["from"]["username"]) + ")");
                }
                else{
